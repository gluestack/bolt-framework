...
import { SEALVM, SSH_CONFIG } from "../constants";
...
async function runProjectInsideVm(
  ...
  if (isDetatched) {
    ...
    return await executeDetachedWithLogs(
      "ssh",
      args,
      join(projectConfig.source, SEALVM.LOG_FOLDER, projectConfig.name),
      {
        shell: true,
        detached: true,
      }
...
export default async function (localPath: string, option: any) {
  try {
    ...
    const project = await validateProjectStatus(sealConfig.projectId, "run");
    ...
    await updateStore("projects", sealConfig.projectId, json);
    ...
    process.on("SIGINT", () => {
      (async () => {
        ...
        await updateStore("projects", sealConfig.projectId, {
          ...project,
          status: "build",
          sshProcessIds: null,
