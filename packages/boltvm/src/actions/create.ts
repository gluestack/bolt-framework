...
export default async (localPath: string, option: any) => {
  try {
    ...
    const container = new Container(
      `${sealvmConfig.projectId}_${sealvmConfig.name}`
    );
    const contianerPath = await container.createImage(cached);
    ...
    await updateStore("projects", sealvmConfig.projectId, json);
  } catch (err: any) {
    console.log(err);
