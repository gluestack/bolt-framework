...
export default async function (localPath: string, options: any) {
  try {
    ...
    await validateProjectStatus(sealConfig.name, "log");

    // Check if .logs folder exists
    const vmLogPath = join(localPath, ".logs");
    ...
    if (!(await exists(outputLogPath))) {
      console.log(
        chalk.red(`> out.logs file not found for ${sealConfig.name}`)
      );
      return;
    }
    if (!(await exists(errorLogPath))) {
      console.log(
        chalk.red(`> err.logs file not found for ${sealConfig.name}`)
      );
      return;
    }
